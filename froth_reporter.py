...
def generate_report():
    ...
    data = {
        ...
        "surface_state": "UNKNOWN", "kit": "CHECK_LOCAL", "ascii_chart": ""
    }
    ...
    try:
        ...
        if len(lines) >= 3:
            ...
            data["wtmp_val"] = raw_wtmp

            # 2. ASCII CHART LOGIC (Visualizing Wave Energy)
            # Create a 5-step gauge based on rideable threshold
            threshold = config.MIN_RIDEABLE_HEIGHT
            bars = 0
            if data["wvht"] > 0:
                bars = int((data["wvht"] / threshold) * 1.5)
                bars = max(1, min(5, bars)) # Keep between 1 and 5
            ...
            elif data["wspd"] > 12: data["surface_state"] = "CHOPPY / BLOWN OUT"
            ...
            # 4. SURF RECOMMENDATION (Updated per Request)
    ...
    # UI ARCHITECTURE: BRUTALIST MINITEL
...
<head>
    ...
    <style>
        ...
        .chart-row {{ display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 2px solid #000; align-items: center; }}
